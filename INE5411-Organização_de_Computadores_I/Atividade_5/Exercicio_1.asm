...
MAIN:
	...
	syscall
	move	$s0, $v0		#Armazena o endereÃ§o da matriz A retornado

	li	$v0, 9			#Código 9 utilizado para alocar memóira
	move	$a0, $t1		#Passa o tamanho da matriz em bytes
	syscall
	move	$s1, $v0		#Armazena o endereÃ§o da matriz B retornado
...
LOOP_COLUNA:
	beq	$t1, MAX, RESET_COLUNA	#Se finalizar a utlima coluna vai chama a funÃ§Ã£o para resetar o valor
	...
	mul	$t2, $t2, 4		#Multiplica esse valor por 4 para encontrar a posiÃ§Ã£o no endereÃ§o
	add 	$t2, $t2,$s0		#Soma a posiÃ§Ã£o encontrada com o endereÃ§o da matriz A
	...
	mul	$t3, $t3, 4		#Multiplica esse valor por 4 para encontrar a posição no endereÃ§o
	add 	$t3, $t3, $s1		#Soma a posição encontrada com o endereÃ§o da matriz B
